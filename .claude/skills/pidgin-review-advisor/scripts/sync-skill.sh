#!/usr/bin/env bash
# Keep every installed copy of pidgin-review-advisor identical.
# The project's .claude/skills copy is the single source of truth.
#
# Targets:
#   <repo>/.agents/skills/...           Antigravity
#   ~/.claude/skills/...                user-level Claude Code install, which
#                                       SHADOWS the project copy — a stale one
#                                       here silently overrides your edits
#
#   sync-skill.sh           copy source -> every target that applies
#   sync-skill.sh --check   report drift, exit 1 if any (CI / pre-commit safe)
set -euo pipefail

SKILL_NAME="pidgin-review-advisor"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
SRC="$REPO_ROOT/.claude/skills/$SKILL_NAME"

# The user-level copy is only synced when it already exists: creating one
