    ...
    const render = () => {
        ...
        grid.innerHTML = filtered.map(item => `
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition group">
                ...
                <h3 class="text-lg font-bold text-gray-600 mb-1">English: <span class="text-gray-900 dark:text-slate-100">${escapeHtml(item.english)}</span></h3>
                <div class="text-2xl font-black text-blue-600 dark:text-blue-400 mb-3">
                    <span class="text-gray-600 text-sm font-normal">Pidgin:</span> ${escapeHtml(item.pidgin)}
                </div>
                ${item.example ? `
                ...
                <div class="mt-4 flex justify-end">
                    <a href="/word/${slugify(item.pidgin)}.html" class="text-xs font-bold text-gray-600 hover:text-blue-600 dark:hover:text-blue-400 transition flex items-center gap-1">
