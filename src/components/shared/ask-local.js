...
class AskLocalManager {
    ...
    async handleFormSubmission() {
        ...
        try {
            ...
            this.showFeedback('Your question has been submitted! Locals will respond soon. <iconify-icon icon="lucide:flower-2"></iconify-icon>', 'success');
    ...
    displayRecentQuestions() {
        ...
        if (questions.length === 0) {
            container.innerHTML = `
                <div class="bg-white p-3 rounded text-center text-gray-500">
                    <p>No questions yet. Be the first to ask! <iconify-icon icon="lucide:flower-2"></iconify-icon></p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = displayQuestions.map(q => {
            ...
            const statusText = hasResponses ? '<iconify-icon icon="lucide:check-circle"></iconify-icon> Answered' : '<iconify-icon icon="lucide:hourglass"></iconify-icon> Pending Response';
                ...
                <div class="bg-white border rounded-lg overflow-hidden">
                    <div class="p-4 border-l-4 ${hasResponses ? 'border-green-400' : 'border-yellow-400'}">
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-2 text-xs text-gray-500">
                                    <span><iconify-icon icon="lucide:user"></iconify-icon> ${this.escapeHtml(q.userName)}</span>
                                    <span>•</span>
                                    <span><iconify-icon icon="lucide:calendar"></iconify-icon> ${this.formatDate(q.timestamp)}</span>
                        ...
                        ${hasResponses ? `
                            <div class="mt-3">
                                <button class="expand-responses-btn text-green-600 hover:text-green-800 text-sm font-medium" data-question-id="${safeId}">
                                    <iconify-icon icon="lucide:eye"></iconify-icon> View ${responseCount} Response${responseCount !== 1 ? 's' : ''} →
                                </button>
                            </div>

                            <!-- Responses Container (initially hidden) -->
                            <div id="responses-container-${safeId}" class="hidden mt-3">
                        ...
                        <div id="response-form-${safeId}" class="hidden mt-4 p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
                            <h5 class="font-semibold mb-3 text-blue-800"><iconify-icon icon="lucide:flower-2"></iconify-icon> Share Your Local Knowledge</h5>
                            <div class="mb-3">
                            ...
                            <div class="mb-3 bg-yellow-50 p-3 rounded border">
                                <label class="block text-sm font-medium mb-1">
                                    <iconify-icon icon="lucide:lock"></iconify-icon> Quick check: <span id="response-captcha-${safeId}"></span> = ?
                                </label>
                            ...
                            <div class="flex gap-2">
                                <button class="submit-response-btn bg-green-700 text-white px-4 py-2 rounded hover:bg-green-800 transition text-sm" data-question-id="${safeId}">
                                    <iconify-icon icon="lucide:flower-2"></iconify-icon> Submit Response
                                </button>
            ...
            container.innerHTML += `
                <div class="text-center mt-6">
                    <a href="ask-local.html" class="inline-block bg-blue-600 text-white px-6 py-3 rounded-full hover:bg-blue-700 transition font-semibold">
                        <iconify-icon icon="lucide:clipboard"></iconify-icon> View All Questions (${questions.length} total)
                    </a>
                </div>
            `;
        }
    ...
    renderResponses(responses) {
        return `
            <div class="bg-green-50 rounded-lg p-3 mt-3 border border-green-200">
                <h5 class="font-semibold text-green-800 mb-2"><iconify-icon icon="lucide:flower-2"></iconify-icon> Local Responses:</h5>
                <div class="space-y-2">
                    ${responses.map(r => {
                        ...
                        <div class="bg-white p-3 rounded border-l-2 border-green-400">
                            ...
                            <div class="flex items-center gap-2 mt-2 text-xs text-gray-500">
                                <span><iconify-icon icon="lucide:user-check"></iconify-icon> ${this.escapeHtml(r.responderName || 'Local Helper')}</span>
                                <span>•</span>
                                <span><iconify-icon icon="lucide:calendar"></iconify-icon> ${this.formatDate(r.timestamp)}</span>
    ...
    toggleResponsesVisibility(questionId, button) {
        ...
        if (responsesContainer) {
            ...
            if (isHidden) {
                ...
                button.textContent = '<iconify-icon icon="lucide:chevron-down"></iconify-icon> Hide Responses';
            } else {
                ...
                button.textContent = `<iconify-icon icon="lucide:eye"></iconify-icon> View ${count} Response${count !== 1 ? 's' : ''} →`;
            }
        }
    }
