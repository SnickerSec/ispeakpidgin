...
async function debugFriend() {
    ...
    const result = await translator.translate('friend', 'eng-to-pidgin');
    console.log('Input: "friend"');
    console.log('Translation:', result.text);
    
    const englishWord = 'friend';
    const options = mockDataLoader.data.translations.englishToPidgin[englishWord] || [];
    const validPidgins = options.map(o => o.pidgin.toLowerCase());
    console.log('Valid options in mock data:', validPidgins);
    console.log('Is match?', validPidgins.includes(result.text.toLowerCase()));
}
